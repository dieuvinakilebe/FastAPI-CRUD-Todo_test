...
# Переопределяем зависимость get_db из database.py,
# чтобы роутеры работали с тестовой БД
def override_get_db():
...
def setup_database():
    # cоздаём таблицы в тестовой БД
    models.Base.metadata.create_all(bind=engine_test)
    yield
    # после всех тестов удаляем временный файл БД
    os.close(TEST_DB_FD)
